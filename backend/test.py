...
def createFactory(testDb, createWorkforceSizeRange) -> Factory:
    factory = Factory(
        name="Test Factory", workforceSizeRange_id=createWorkforceSizeRange.id
    )
    testDb.add(factory)
    testDb.commit()
...
def createIncident(
    ...
    createThreatType,
    createWorkType,
    createIndustryTypeLarge,
    createIndustryTypeMedium,
    createWorker,
) -> Incident:
    incident = Incident(
...
def testFactoryModel(testDb, createFactory):
    ...
    retrievedFactory = (
        testDb.query(Factory).filter(Factory.id == createFactory.id).first()
    )
...
def testIncidentModel(testDb, createIncident, createFactory):
    ...
    retrievedIncident = (
        testDb.query(Incident).filter(Incident.id == createIncident.id).first()
    )
...
def testGetIncidentsByFactory(testDb, createIncident, createFactory):
    ...
    assert len(data["incidents"]) >= 1
    assert all(
        incident["factory"]["id"] == createFactory.id for incident in data["incidents"]
    )
...
def test_risk_assessment_success(testDb, sample_payload):
    response = client.post("/api/risk-assessment", json=sample_payload)
    ...
    assert bi["date"].startswith("2025-05-11")
    assert bi["name"] == sample_payload["이름"]
    assert bi["factory"] == sample_payload["발생공장"]
    assert bi["image"] == sample_payload["사진"]
    assert bi["description"] == sample_payload["설명"]

    ap = data["accident_possibility"]
    assert isinstance(ap["risk_score"], int)
    assert ap["risk_level"] in {"Low", "Medium", "High"}
    assert isinstance(ap["predicted_probability"], float)

    ms = data["micro_accident_severity"]
    assert isinstance(ms["probabilities"], dict)
    assert isinstance(ms["most_likely_time"], str)

    saved = testDb.query(RiskAssessment).first()
    assert saved is not None
    assert saved.to_dict() == data


def test_risk_assessment_missing_field(testDb):
    bad_payload = {
        "날짜": "2025-05-11",
        "이름": "홍길동",
        "발생공장": "A제조공장",
    ...
    response = client.post("/api/risk-assessment", json=bad_payload)
